...
const createTicket = async (req, res)=>{
    ...
    if (pdfBuffer.length > 5 * 1024 * 1024) {
        return error422("File size must be under 5 MB", res);
    }

    // ?? DETECT FILE TYPE
    const fileTypeResult = await FileType.fromBuffer(pdfBuffer);

    if (!fileTypeResult) {
        return error422("Invalid or corrupted file", res);
    }
    ...
    if (!allowedMimeTypes.includes(fileTypeResult.mime)) {
        return error422("Only PDF, JPG, PNG files are allowed", res);
    }
    ...
    await connection.query(
        ...
        (ticket_id, ticket_conversation_id, file_path, uploaded_by)
        VALUES (?, ?, ?, ?)`,
...
const updateTicket = async (req, res) => {
    ...
    const fileName = `ticket_${ticketId}_${Date.now()}.${fileTypeResult.ext}`;
    const filePath = path.join(__dirname, "..", "..", "uploads", fileName);
    ...
    await connection.query(
        ...
        <p>Closed At: ${closed_at}</p>
          <p>If you are satisfied with the solution, no further action is required.</p>
        ...
        if (ticket_status === "Re-assign") {
        // send email technician
        const userQuery = `SELECT u.user_name, u.email_id FROM users u
        LEFT JOIN customer_agents ca ON ca.user_id = u.user_id
        WHERE u.role_id = 2 AND u.status = 1 AND ca.customer_id = ${customer_id}`;
        const [userResult] = await connection.query(userQuery);
...
const getAllTickets = async (req, res) => {
    ...
    try {
        ...
        // }
        if (user_id) {
            getTicketsQuery += ` AND ((ta.assigned_to IS NULL AND ca.user_id = ${user_id}) OR ta.assigned_to = ${user_id} OR (t.user_id = ${user_id} AND t.customer_user_id = ${user_id}) OR t.ticket_status = 'Re-assign' )`;
            countQuery += ` AND ((ta.assigned_to IS NULL  AND ca.user_id = ${user_id}) OR ta.assigned_to = ${user_id} OR (t.user_id = ${user_id} AND t.customer_user_id = ${user_id})  OR t.ticket_status = 'Re-assign')`;
        }
...
const getTicketStatusCountold = async (req, res) => {
    ...
    try {
        ...
        if (user_id) {
            statusCountQuery += ` AND ((ta.assigned_to IS NULL AND ca.user_id = ${user_id}) OR ta.assigned_to = ${user_id} OR t.user_id = ${user_id})`;
        }
        if (customer_id) {
        ...
        if (user_id) {
            statusCountQuery += `
                AND (
                    ...
                    ta.assigned_to = ${user_id} OR t.user_id = ${user_id} OR t.customer_user_id = ${user_id}
...
const getStatusList = async (req, res) => {
    ...
    try {
        ...
        if (user_id) {
            statusListQuery += ` AND ((ta.assigned_to IS NULL AND ca.user_id = ${user_id}) OR ta.assigned_to = ${user_id} OR t.user_id = ${user_id} OR t.customer_user_id = ${user_id} )`;
            countQuery += ` AND ((ta.assigned_to IS NULL AND ca.user_id = ${user_id}) OR ta.assigned_to = ${user_id} OR t.user_id = ${user_id} OR t.customer_user_id = ${user_id} )`;
        }
